...
// Complete ISO 3166-1 alpha-2 country dataset (249 officially assigned entries)
export const ISO_3166_1_COUNTRIES: IsoCountry[] = [
  ...
  { name: "Holy See (Vatican City)", code: "VA", dialCode: "+39", flag: "🇻🇦" },
...
// Fast O(1) lookups
const COUNTRY_BY_CODE = new Map<string, IsoCountry>();
const COUNTRIES_BY_DIAL_CODE = new Map<string, IsoCountry[]>();

for (const country of ISO_3166_1_COUNTRIES) {
...
export function findCountriesByDialCode(dialCode?: string): IsoCountry[] {
  if (!dialCode) return [];
  const clean = dialCode.startsWith('+') ? dialCode : `+${dialCode}`;
  return COUNTRIES_BY_DIAL_CODE.get(clean) || [];
}

export function searchCountries(query: string): IsoCountry[] {
  if (!query || !query.trim()) return ISO_3166_1_COUNTRIES;
  const q = query.toLowerCase().trim();
  return ISO_3166_1_COUNTRIES.filter(c =>
...
export interface E164ValidationResult {
  ...
  country?: IsoCountry;
  error?: string;
  errorCode?: 'INVALID_COUNTRY' | 'INVALID_NUMBER' | 'MISSING_CALLING_CODE' | 'MALFORMED_E164';
}

/**
 * Validates and normalizes phone numbers to standard E.164 (+[1-9]\d{6,14})
 * without assuming any default country or India (+91).
 */
export function validateAndNormalizeE164(
  rawNumber?: string,
  options?: {
    countryCode?: string;
    dialCode?: string;
  }
): E164ValidationResult {
  ...
  // If a countryCode is passed, verify it is valid ISO 3166-1
  let country: IsoCountry | undefined;
  if (options?.countryCode) {
    country = findCountryByCode(options.countryCode);
    if (!country) {
      return {
        valid: false,
        error: `Invalid country: "${options.countryCode}" is not a recognized ISO 3166-1 country code.`,
  ...
  // Determine dialCode
  const dialCode = country?.dialCode || options?.dialCode;
  ...
  if (trimmed.startsWith('+')) {
    ...
    if (!digitsOnly || digitsOnly.length < 7 || digitsOnly.length > 15) {
      return {
        valid: false,
        error: `Malformed E.164 number: "${trimmed}" must contain between 7 and 15 digits following "+".`,
        errorCode: 'MALFORMED_E164'
      };
    }

    if (/^0+$/.test(digitsOnly) || digitsOnly.startsWith('0')) {
      return {
        valid: false,
        error: `Malformed E.164 number: country code cannot start with 0.`,
        errorCode: 'MALFORMED_E164'
      };
    }
    ...
    if (!e164Regex.test(e164)) {
      return {
        valid: false,
        error: `Malformed E.164 phone number "${e164}". Standard format is +[country_code][national_number].`,
        errorCode: 'MALFORMED_E164'
      };
    }

    // Try to match country by dialing prefix if not provided
    if (!country) {
      for (let len = 4; len >= 1; len--) {
        const prefix = `+${digitsOnly.slice(0, len)}`;
        const matched = COUNTRIES_BY_DIAL_CODE.get(prefix);
  ...
  if (!rawDigits || rawDigits.length < 5) {
    return {
      valid: false,
      error: 'Invalid phone number: number contains insufficient digits.',
      errorCode: 'INVALID_NUMBER'
    };
  }

  // Reject dummy repeating numbers
  if (/^0+$/.test(rawDigits) || /^1+$/.test(rawDigits)) {
    return {
      valid: false,
  ...
  if (!dialCode) {
    return {
      valid: false,
      error: 'Missing calling code: Destination number must include international calling code (e.g. +1, +44, +91) or select a country.',
      errorCode: 'MISSING_CALLING_CODE'
    };
  }
  ...
  if (!cleanDialDigits) {
    return {
      valid: false,
      error: 'Missing calling code: invalid international calling code.',
      errorCode: 'MISSING_CALLING_CODE'
    };
  }

  // Strip single national trunk prefix '0' if present (e.g. UK 07911... -> 7911...)
  let nationalNumber = rawDigits;
  if (nationalNumber.startsWith('0') && nationalNumber.length > 6) {
    nationalNumber = nationalNumber.replace(/^0+/, '');
  }

  // If user entered full international number including dial code prefix
  let combinedDigits: string;
  if (nationalNumber.startsWith(cleanDialDigits) && nationalNumber.length > cleanDialDigits.length + 5) {
    combinedDigits = nationalNumber;
  } else {
    combinedDigits = `${cleanDialDigits}${nationalNumber}`;
  }

  const e164 = `+${combinedDigits}`;
  const e164Regex = /^\+[1-9]\d{6,14}$/;
  if (!e164Regex.test(e164)) {
    return {
      valid: false,
      error: `Malformed E.164 phone number "${e164}". E.164 standard requires 7 to 15 digits.`,
      errorCode: 'MALFORMED_E164'
    };
  }
  ...
  return {
    valid: true,
    e164Number: e164,
    country: country || (COUNTRIES_BY_DIAL_CODE.get(dialCode)?.[0])
  };
}
