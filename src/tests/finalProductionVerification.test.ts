...
describe('2. Gmail Account Resolution & Send Safety Verification', () => {
  ...
  it('rejects mock_access_token in production mode', async () => {
    ...
    const mockAccount = { email: 'user@company.com', accessToken: 'mock_access_token_123' };
    await expect(sendGmailMessage(mockAccount, 'test@example.com', 'Sub', 'Body', true))
      .rejects.toThrow('Real Gmail OAuth connection required');
  });

  it('resolves authorized account strictly by verified organizationId', async () => {
    const mockGoogleAccountsDb = [
      { organization_id: 'org_alpha', email: 'alpha@company.com', access_token: 'valid_alpha_token' },
      { organization_id: 'org_beta', email: 'beta@company.com', access_token: 'valid_beta_token' }
    ];
