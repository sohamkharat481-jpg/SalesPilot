  ...
  return (
    <div className="space-y-6 sm:space-y-8 pb-12 animate-fade-in max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 pt-2 sm:pt-6 min-w-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 sm:mb-6">
            <div>
              <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Revenue & Growth Tracker</h3>
        ...
        <div className="lg:col-span-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm flex flex-col justify-between">
          <div>
            <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white mb-1">Lead Sources</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Acquisition channels breakdown</p>
          </div>
          <div className="h-44 sm:h-48 my-3 sm:my-4">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={leadSources}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <div>
              <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Recent Leads Feed</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Newly discovered prospects in your pipeline</p>
            </div>
            <button
              onClick={() => setActiveTab('leads')}
              className="px-3 sm:px-4 py-1.5 sm:py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-xs rounded-xl transition-all shrink-0"
            >
              View All Leads
            </button>
          </div>
          {recentLeads.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentLeads.map((lead) => (
                <div key={lead.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3.5 sm:p-4 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800/80 hover:border-blue-500/30 transition-all gap-2 sm:gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 font-bold text-xs flex items-center justify-center shrink-0">
                      {lead.name ? lead.name.substring(0, 2).toUpperCase() : 'LD'}
                    </div>
                    <div className="min-w-0 flex-1">
                      <h4 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white truncate">{lead.name || lead.company}</h4>
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{lead.email || lead.phone || 'No contact info'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2.5 self-end sm:self-auto shrink-0">
                    <span className={`px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] font-bold ${
                    ...
                    <button
                      onClick={() => setActiveTab('leads')}
                      className="p-1.5 sm:p-2 hover:bg-white dark:hover:bg-slate-700 rounded-xl text-slate-400 hover:text-blue-600 transition-all"
                    >
        ...
        <div className="lg:col-span-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Upcoming Meetings</h3>
              <button
            ...
            {upcomingMeetings.length === 0 ? (
              ...
              <div className="space-y-3">
                {upcomingMeetings.map((apt) => (
                  <div key={apt.id} className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                    <div className="flex items-center justify-between mb-1 gap-2">
                      <span className="text-xs font-bold text-slate-900 dark:text-white truncate">{apt.title || apt.leadName}</span>
                      <span className="text-[10px] font-semibold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/50 px-2 py-0.5 rounded-full shrink-0">
                        {new Date(apt.dateTime || apt.startTime || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{apt.email || apt.company}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="mt-4 sm:mt-6 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-slate-800/50 dark:to-blue-950/30 border border-blue-100 dark:border-slate-800">
            <div className="flex items-center gap-2.5 mb-1.5 sm:mb-2">
              <Bot className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 dark:text-blue-400 animate-pulse shrink-0" />
