...
import { 
  ...
  CloudLightning, RefreshCw, Zap, Server, Award
} from 'lucide-react';
...
export function LeadsView({ 
  ...
  useEffect(() => {
    const saved = localStorage.getItem('selected_lead_id');
    if (saved) {
  ...
  const handleCreateDeal = async (e?: React.FormEvent, forceDuplicate = false) => {
    ...
    try {
      ...
      const body = {
        ...
        name: dealForm.name || `Deal - ${selectedLead.company || selectedLead.firstName}`,
        company: selectedLead.company,
        contactName: `${selectedLead.firstName} ${selectedLead.lastName}`,
        value: Number(dealForm.value),
        currency: dealForm.currency,
        stage: 'QUALIFIED',
        expectedCloseDate: dealForm.expectedCloseDate || undefined,
        assignedUserId: dealForm.assignedUserId || undefined,
      ...
      if (response.status === 409) {
        if (confirm('An active deal already exists for this lead. Are you sure you want to create a duplicate opportunity?')) {
          await handleCreateDeal(undefined, true);
        }
        return;
      }
      ...
      alert(`Deal for "${createdDeal.leadName}" created successfully!`);
      ...
      // Reload lead list to reflect the link with dealId
      await fetchPersistedLeads();
  ...
  return (
    <div id="leads_module_root" className="space-y-8 animate-fade-in text-slate-900 dark:text-slate-100">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      ...
                      <button
                        onClick={() => {
                          setDealForm({
                            name: `Deal - ${selectedLead.company || selectedLead.firstName}`,
                            value: 50000,
                            currency: 'INR',
                            expectedCloseDate: new Date(Date.now() + 30 * 24 * 3600 * 1000).toISOString().split('T')[0],
                            assignedUserId: (selectedLead as any).assignedUserId || '',
                            description: `Opportunity created for ${selectedLead.firstName} ${selectedLead.lastName}`,
                            nextAction: 'Schedule introduction follow-up'
                          });
                          setShowCreateDealModal(true);
                        }}
                        className="px-2 py-1 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white rounded-lg text-[10px] font-semibold flex items-center gap-1 shadow-sm transition-all cursor-pointer"
                      >
      ...
      {/* 4. CREATE DEAL MODAL */}
      {showCreateDealModal && selectedLead && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl w-full max-w-lg p-6 space-y-4 animate-slide-up">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Creating an active deal for <span className="font-semibold text-slate-800 dark:text-slate-200">{selectedLead.firstName} {selectedLead.lastName}</span> at <span className="font-semibold text-slate-800 dark:text-slate-200">{selectedLead.company}</span>
                </p>
              </div>
              <button 
            ...
            <form onSubmit={(e) => handleCreateDeal(e)} className="space-y-4 text-xs">
              <div>
                <label className="block text-[10px] font-mono font-bold uppercase text-slate-400 mb-1">Opportunity / Deal Name *</label>
                <input 
                  type="text" 
                  required
                  placeholder="e.g. Enterprise Outreach Expansion"
                  value={dealForm.name}
                  onChange={(e) => setDealForm({ ...dealForm, name: e.target.value })}
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 p-2.5 rounded-lg focus:outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[10px] font-mono font-bold uppercase text-slate-400 mb-1">Currency *</label>
                  <select 
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[10px] font-mono font-bold uppercase text-slate-400 mb-1">Assigned Sales Representative</label>
                  <select 
                    value={dealForm.assignedUserId}
                    onChange={(e) => setDealForm({ ...dealForm, assignedUserId: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 p-2.5 rounded-lg focus:outline-none"
                  >
                    <option value="">Select teammate...</option>
                    {teamMembersList.map(tm => (
                      <option key={tm.id} value={tm.id || tm.userId}>{tm.fullName} ({tm.role})</option>
                    ))}
                  </select>
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-mono font-bold uppercase text-slate-400 mb-1">Description / Notes</label>
                <textarea 
                  rows={3}
                  placeholder="Summarize the deal context, business objectives, pain points, timeline..."
              ...
              <div>
                <label className="block text-[10px] font-mono font-bold uppercase text-slate-400 mb-1">Next Immediate Action</label>
                <input 
                  type="text" 
                  placeholder="e.g. Schedule a custom product pricing walkthrough"
              ...
              {dealForm.assignedUserId && (
                <div className="p-2.5 bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/40 rounded-lg text-[10px] text-blue-700 dark:text-blue-300 flex items-center gap-1.5 font-medium">
                  <CheckCircle2 className="w-3.5 h-3.5 text-blue-500" />
                  <span>Opportunity will be assigned to <strong>{teamMembersList.find(tm => tm.id === dealForm.assignedUserId || tm.userId === dealForm.assignedUserId)?.fullName || 'the selected agent'}</strong>.</span>
                </div>
              )}
              ...
              <div className="flex gap-2 pt-2">
                <button 
                  type="button" 
                  onClick={() => setShowCreateDealModal(false)}
                  className="flex-1 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-xs font-bold text-slate-700 dark:text-slate-300 rounded-lg transition cursor-pointer"
                >
                  Cancel
                </button>
                <button 
                  type="submit" 
                  className="flex-1 py-2.5 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-xs font-bold text-white rounded-lg transition shadow-sm cursor-pointer"
                >
                  Create Opportunity
                </button>
              </div>
