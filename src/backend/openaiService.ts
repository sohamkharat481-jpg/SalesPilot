...
export async function generateFallbackAICompletion(
  ...
  if (systemPrompt.includes('copywriter') || userPrompt.includes('cold email')) {
    simulatedText = `Subject: Scaling outbound operations\n\nHi,\n\nI noticed your organization is actively scaling operations and expanding market reach.\n\nWhen accelerating customer acquisition, maintaining high outreach quality without consuming hours on manual research is a common bottleneck. SalesPilot automates these bespoke research dossiers and outreach tracks, saving over 15 hours per week while protecting deliverability.\n\nWould you be open to a brief, 10-minute demo this Thursday at 3 PM to see how we streamline this for your team?\n\nBest regards,\nSales Operations`;
  } else if (systemPrompt.includes('research') || userPrompt.includes('Conduct sales research') || systemPrompt.includes('Research')) {
    simulatedText = `### Sales Research Dossier\n\n**1. Core Value Proposition:**\nActive commercial enterprise with verified market presence and growing customer operations. Focused on scaling service delivery while optimizing internal administrative and client acquisition workflows.\n\n**2. Core Operational Pain Points:**\n- Outdated or fragmented tool stacks with manual pipeline synchronization.\n- Significant time sink: Sales and ops teams spending hours manually conducting prospect research.\n- Opportunity loss from delayed response times and unoptimized outreach sequences.\n\n**3. Custom Personalization Hooks & Icebreakers:**\n- *"Noticed your steady expansion and active regional footprint. How is your team managing outbound sales capacity?"*\n- *"Congratulations on your recent operational milestones. Scaling brings pipeline syncing friction across disparate CRM tools."*\n- *"Saw your recent initiatives in market expansion. How are you maintaining outreach conversion standards?"*`;
  } else if (systemPrompt.includes('objection') || userPrompt.includes('objection')) {
    simulatedText = `### Sales Coach Review & Score: 8.5/10\n\n**Psychological Friction Points Analyzed:**\nThe prospect is showing standard "Risk-Aversion" and "Inertia" objections. Recommending a heavy discount immediately damages your premium brand value and frames your tool as a low-margin option. Instead, shift focus to their operational ROI.\n\n**High-Impact Corrective Objection Line:**\n*"I completely understand. A budget constraint is a real priority. But let's look at this differently: if your current team is spending 2 hours per day manually compiling leads and research, that is roughly 40 hours of high-value team time lost per month. SalesPilot recovers those 40 hours for ₹8,500—essentially paying for itself in the first 4 days of automation. Let's schedule a pilot to verify this ourselves, risk-free. What do you think?"*`;
  } else if (systemPrompt.includes('BANT') || userPrompt.includes('Qualify')) {
    simulatedText = `### BANT Qualification Score: VERY HOT (92% Confidence)\n\n**BANT Vector Analysis:**\n- **Budget (B):** Verified. Active commercial operations with healthy cash flows, expanding headcounts, and budget for operational software.\n- **Authority (A):** High. Contact holds direct leadership or director oversight, shortening the decision hierarchy to a single call.\n- **Need (N):** Urgent. Experiencing active business scale with bottlenecks in manual prospecting and pipeline management.\n- **Timeline (T):** Immediate. Ready to implement within the next 10-14 business days.\n\n**Immediate recommendation:** Fast-track to a Google Meet booking slot and offer a custom sandbox environment to build momentum.`;
  } else if (systemPrompt.includes('intent') || userPrompt.includes('reply')) {
    simulatedText = `### Reply Analysis & Category\n\n**1. Intent Classification:** \`BOOKING_REQUEST\` (95% Probability)\n**2. Emotional Sentiment:** Warm & Receptive\n\n**Response Recommendations:**\nThe lead has accepted the booking slot and provided details of their availability ("this Thursday 3 PM"). Immediately dispatch a secure Google Calendar reservation, attach a live Google Meet conference room link, and log a pre-demo activity in their timeline.\n\n**Proposed reply template:**\n*"Fantastic! I have locked in Thursday at 3 PM IST. I just sent a Google Calendar invite with our conference room link. Looking forward to showing you how we can automate this outreach workload!"*`;
    ...
    simulatedText = `# Enterprise Sales Pilot Agreement & Proposal\n\n**Deal value:** ₹50,000 INR (6-Month License)\n\n---\n\n### 1. Executive Summary\nScaling customer acquisition framework. SalesPilot will deliver enterprise licenses, automating outbound deliverability and equipping teams with server-side AI-personalization sequencers.\n\n### 2. Tailored Solutions & Deliverables\n- **Enterprise Platform Access:** Full CRM integration, Google Calendar synchronization, and automated sequence triggers.\n- **Zero-Bounce Lead Generation:** Live verified lead generation engine across targeted industries and territories.\n- **B2B Research Intelligence:** High-performance server-side LLM modules for cold copy composition and instant lead enrichment.\n\n### 3. Financial Agreement (INR)\n- **Pilot Term Fee:** ₹50,000 INR inclusive of all platform server hosting fees.\n- **Billing terms:** 100% upfront activation. Cashfree PG link provided.`;
    ...
    simulatedText = `Subject: Quick follow up / Outreach efficiency metrics\n\nHi,\n\nI wanted to share a quick metric with you—we recently helped a commercial enterprise automate their outreach loops. They reduced copywriting and research time by 85% while their positive meeting bookings increased from 1.2% to 4.8%.\n\nAre you still available to take a look at the live SalesPilot setup this Thursday?\n\nBest regards`;
